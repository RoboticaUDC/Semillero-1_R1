...
class Pose:
    ...
    NEUTRAL = np.array([
        0.20,  0.20,  0.00,  1.28,  0.00,   # brazo izquierdo
        0.20, -0.20,  0.00,  1.28,  0.00,   # brazo derecho
    ], dtype=np.float32)

    # Brazo derecho levantado, listo para saludar
    WAVE_READY = np.array([
        0.20,  0.20,  0.00,  1.28,  0.00,   # izq: neutral
        0.35, -1.56, -1.26, -0.40,  0.00,   # der: levantado
    ], dtype=np.float32)

    # Brazo derecho agitando (codo flexionado)
    WAVE_UP = np.array([
        0.20,  0.20,  0.00,  1.28,  0.00,   # izq: neutral
        0.35, -1.56, -1.26,  1.07,  0.00,   # der: codo arriba
    ], dtype=np.float32)

    # Apuntar con brazo derecho hacia adelante
    POINT_RIGHT = np.array([
        0.20,  0.20,  0.00,  1.28,  0.00,   # izq: neutral
       -0.10, -0.20,  0.00,  0.05,  0.00,   # der: extendido al frente
    ], dtype=np.float32)

    # Apuntar con ambos brazos hacia adelante (pose de carga)
    CARRY = np.array([
       -0.10,  0.20,  0.00,  0.30,  0.00,   # izq: extendido frente
       -0.10, -0.20,  0.00,  0.30,  0.00,   # der: extendido frente
    ], dtype=np.float32)

    # Brazos en cruz (horizontal)
    CROSS = np.array([
        0.00,  1.57,  0.00,  0.10,  0.00,   # izq: horizontal
        0.00, -1.57,  0.00,  0.10,  0.00,   # der: horizontal
    ], dtype=np.float32)

    # Pose de guardia (boxeo ligero)
    GUARD = np.array([
        0.60,  0.50,  0.00,  1.20,  0.00,   # izq: guardia alta
        0.60, -0.50,  0.00,  1.20,  0.00,   # der: guardia alta
    ], dtype=np.float32)

    # Brazos hacia abajo, relajados
    RELAXED = np.array([
        0.60,  0.10,  0.00,  1.50,  0.00,   # izq: caídos
        0.60, -0.10,  0.00,  1.50,  0.00,   # der: caídos
    ], dtype=np.float32)
...
class ArmController:
    ...
    ARM_INDICES = np.arange(14, 24)   # joints 15..22

    # Ganancias PD para los brazos
    KP = np.array([80, 60, 60, 80, 20, 80, 60, 60, 80, 20], dtype=np.float32)
    KD = np.array([ 3,  2,  2,  2,  1,  3,  2,  2,  2,  1], dtype=np.float32)

    # Límites de torque por joint (Nm)
    TORQUE_LIMIT = np.array([25, 25, 25, 25, 25, 25, 25, 25, 25, 25], dtype=np.float32)

    # Límites articulares (rad) — aproximados para G1
    JOINT_LOWER = np.array([-3.14, -0.23, -1.92, -0.98, -1.92, -3.14, -2.48, -1.92, -0.98, -1.92], dtype=np.float32)
    JOINT_UPPER = np.array([ 2.09,  2.48,  1.92,  2.19,  1.92,  2.09,  0.23,  1.92,  2.19,  1.92], dtype=np.float32)
    ...
    def set_manual(self, joint_idx: int, delta: float):
        ...
        names = ["L-Pitch", "L-Roll", "L-Yaw", "L-Elbow", "L-Wrist",
                 "R-Pitch", "R-Roll", "R-Yaw", "R-Elbow", "R-Wrist"]
    ...
    def update(self, manual_target: np.ndarray | None = None):
        ...
        if self._active_seq is not None and self._active_seq.active:
            ...
            self.target_pose[5] = manual_target[0]   # shoulder pitch
            self.target_pose[6] = manual_target[1]   # shoulder roll
            self.target_pose[7] = manual_target[2]   # shoulder yaw
            self.target_pose[8] = manual_target[3]   # elbow
            self.target_pose[9] = manual_target[4]   # wrist roll
    ...
    def apply(self, data):
        ...
        q  = data.qpos[21:31].astype(np.float32)   # qpos[21..30] R1 brazos
        qd = data.qvel[20:30].astype(np.float32)   # qvel[20..29] R1 brazos
